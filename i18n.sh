...
function usage () {
    ...
    echo "  update winguhub           update po of winguhub"
    echo "  update-all              update po of all apps"
    echo "  compile <app>           compile po file of this app"
    echo "  compile winguhub          compile po file of winguhub"
...
case $1 in
    update)
        ...
        ;;
    update-all)
        printf "\033[1;32m[i18n]\033[m >>>>>  update po of winguhub <<<<<\n"
        ${django_admin} makemessages -l zh_CN -e py,html \
            -i "thirdpart*" -i "api*" -i "avatar*" -i "base*" \
            -i "contacts*" -i "group*" -i "notifications*" -i "organizations*" \
            -i "profile*" -i "share*" -i "media*"
    ...
    compile)
        ...
        ;;
    compile-all)
        printf "\033[1;32m[i18n]\033[m >>>>>  compile po of winguhub <<<<<\n"
        ${django_admin} compilemessages
        for app in ${apps}; do
