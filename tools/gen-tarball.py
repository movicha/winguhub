...
def main():
    ...
    tarball_name = 'winguhub-%s.tar.gz' % version
    tmp_tarball = os.path.join(tmpdir, tarball_name)

    cmd = 'git archive --prefix=winguhub-%(version)s/ -o %(tarball)s %(branch)s' \
    ...
    winguhub_dir = os.path.join(tmpdir, 'winguhub-%s' % version)
    if run('./i18n.sh compile-all', cwd=winguhub_dir) != 0:
        error('failed to compile messages')

    with open(os.path.join(winguhub_dir, 'latest_commit'), 'w') as fp:
        fp.write(latest_commit_info)
        fp.write('\n')

    if run('tar czvf %s winguhub-%s' % (tarball_name, version)) != 0:
