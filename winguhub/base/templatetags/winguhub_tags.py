...
from winguhub.profile.settings import NICKNAME_CACHE_TIMEOUT, NICKNAME_CACHE_PREFIX, \
    EMAIL_ID_CACHE_TIMEOUT, EMAIL_ID_CACHE_PREFIX
from winguhub.cconvert import CConvert
from winguhub.po import TRANSLATION_MAP
from winguhub.shortcuts import get_first_object_or_none
from winguhub.utils import normalize_cache_key
...
def translate_commit_desc(value):
    ...
    if value.startswith('Reverted repo'):
        ...
        # only used inside of wingufile system.
    ...
    if value.startswith('Reverted library'):
        ...
        return _('Auto merge by wingufile system')
    else:
...
@register.filter(name='translate_winguhub_time')
def translate_winguhub_time(value):
    """Translate winguhub time to human friendly format instead of timestamp"""
...
@register.filter(name='winguhub_urlize')
def winguhub_urlize(value, autoescape=None):
    """Converts URLs in plain text into clickable links."""
    from winguhub.base.utils import urlize
    return mark_safe(urlize(value, nofollow=True, autoescape=autoescape))
winguhub_urlize.is_safe=True
winguhub_urlize.needs_autoescape = True
