...
from winguhub.wiki import get_group_wiki_repo, get_group_wiki_page, convert_wiki_link,\
    get_wiki_pages
from winguhub.wiki.models import WikiDoesNotExist, WikiPageMissing, GroupWiki
from winguhub.wiki.utils import clean_page_name, get_wiki_dirent
from winguhub.settings import SITE_ROOT, SITE_NAME, MEDIA_URL
...
from winguhub.utils import calc_file_path_hash
from winguhub.utils.paginator import Paginator
from winguhub.views import is_registered_user
from winguhub.forms import SharedRepoCreateForm
    ...
    if request.method == 'POST':
        ...
        if num_of_groups > 0:
            ...
            if current_groups > num_of_groups:
                result['error'] = _(u'You can only create %d groups.<a href="http://wingufile.com/">Upgrade account.</a>') % num_of_groups
        ...
        if request.cloud_mode:
            ...
            if group_members > 0:
                ...
                if current_group_members > group_members:
                    result['error'] = _(u'You can only invite %d members.<a href="http://wingufile.com/">Upgrade account.</a>') % group_members
                    return HttpResponse(json.dumps(result), status=500,
                                        content_type=content_type)
...
def attention(request):
    ...
    for m in members:
        ...
        from winguhub.base.templatetags.winguhub_tags import email2nickname, char2pinyin
