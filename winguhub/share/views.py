...
from winguhub.signals import share_file_to_user_successful
from winguhub.views import validate_owner, is_registered_user
from winguhub.utils import render_permission_error, string2list, render_error, \
...
def share_to_public(request, repo, permission):
    ...
    repo_id = repo.id
    try:
        wingufile_api.add_inner_pub_repo(repo_id, permission)
    except Exception, e:
        logger.error(e)
...
def share_to_group(request, repo, from_user, group, permission):
    ...
    if repo.id in wingufile_api.get_group_repoids(group.id):
    ...
    try:
        wingufile_api.group_share_repo(repo_id, group_id, from_user, permission)
    except Exception, e:
        logger.error(e)
...
def share_to_user(request, repo, from_user, to_user, permission):
    ...
    if is_registered_user(to_user):
        try:
            wingufile_api.share_repo(repo_id, from_user, to_user, permission)
        except Exception, e:
            logger.error(e)
...
def check_user_share_quota(username, repo, users=[], groups=[]):
    ...
    check_pass = False
    quota = wingufile_api.get_user_quota(username)
    self_usage = wingufile_api.get_user_self_usage(username)
    current_share_usage = wingufile_api.get_user_share_usage(username)

    share_usage = 0
    if users:
        share_usage += wingufile_api.get_repo_size(repo.id) * (len(users))
    ...
    if groups:
        ...
        share_usage += wingufile_api.get_repo_size(repo.id) * (len(grp_members) -1)
...
def share_repo(request):
    ...
    repo = wingufile_api.get_repo(repo_id)
    if not repo:
        raise Http404
    ...
    if not check_user_share_quota(from_email, repo, users=share_to_users,
        ...
        messages.error(request, _('Failed to share "%s", no enough quota. <a href="http://wingufile.com/">Upgrade account.</a>') % repo.name)
        return HttpResponseRedirect(next)
...
def list_share_out_repos(request):
    ...
    shared_repos += wingufile_api.get_share_out_repo_list(username, -1, -1)
...
def list_priv_shared_files(request):
    ...
    for e in priv_share_out:
        ...
        e.repo = wingufile_api.get_repo(e.repo_id)
...
def send_shared_link(request):
    ...
    from winguhub.settings import SITE_NAME
...
def user_share_list(request, id_or_email):
    ...
    share_list = []
    username = request.user.username
    share_in = wingufile_api.get_share_in_repo_list(username, -1, -1)
    for e in share_in:
        if e.share_type == 'personal' and e.user == to_email:
            e.share_in = True
            share_list.append(e)
    share_out = wingufile_api.get_share_out_repo_list(username, -1, -1)    
    for e in share_out:
        if e.share_type == 'personal' and e.user == to_email:
            e.share_out = True
            share_list.append(e)
