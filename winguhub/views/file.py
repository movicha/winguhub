...
from winguhub.base.decorators import repo_passwd_set_required
from winguhub.contacts.models import Contact
from winguhub.share.models import FileShare, PrivateFileDirShare
from winguhub.wiki.utils import get_wiki_dirent
from winguhub.wiki.models import WikiDoesNotExist, WikiPageMissing
from winguhub.utils import show_delete_days, render_error, \
...
from winguhub.utils.file_types import (IMAGE, PDF, IMAGE, DOCUMENT, MARKDOWN, \
                                         TEXT, SF)
from winguhub.utils import HAS_OFFICE_CONVERTER

if HAS_OFFICE_CONVERTER:
    from winguhub.utils import query_office_convert_status, query_office_file_pages, \
        prepare_converted_html, OFFICE_PREVIEW_MAX_SIZE

from winguhub.settings import FILE_ENCODING_LIST, FILE_PREVIEW_MAX_SIZE, \
    FILE_ENCODING_TRY_LIST, USE_PDFJS, MEDIA_URL, SITE_ROOT
from winguhub.views import is_registered_user, get_repo_access_permission, \
...
def view_file(request, repo_id):
    ...
    if exceeds_limit:
        ...
        if is_textual_file(file_type=filetype):
            ...
            dirs = wingufile_api.list_dir_by_commit_and_path(current_commit.id, parent_dir)
            if not dirs:
                raise Http404
...
def file_edit_submit(request, repo_id):
    ...
    if repo.encrypted:
        repo.password_set = wingufile_api.is_password_set(repo_id, username)
...
def file_edit(request, repo_id):
    ...
    if filetype == TEXT or filetype == MARKDOWN or filetype == SF: 
        if repo.encrypted:
            repo.password_set = wingufile_api.is_password_set(repo_id, request.user.username)
            if not repo.password_set:
                op = 'decrypt'
        if not op:
...
def view_priv_shared_file(request, token):
    ...
    obj_id = wingufile_api.get_file_id_by_path(repo.id, path)
    if not obj_id:
        raise Http404
    ...
    access_token = wingufile_api.get_httpserver_access_token(repo.id, obj_id,
                                                           'view', username)
    raw_path = gen_file_get_url(access_token, filename)
    inner_path = gen_inner_file_get_url(access_token, filename)
