...
from winguhub.utils import IS_EMAIL_CONFIGURED
from winguhub.forms import SetUserQuotaForm, AddUserForm
...
from winguhub.settings import INIT_PASSWD, \
...
def sys_repo_admin(request):
    ...
    repos_all = wingufile_api.get_repo_list(per_page * (current_page -1),
                                          per_page + 1)
    ...
    for repo in repos:
        try:
            repo.owner = wingufile_api.get_repo_owner(repo.id)
        except:
...
def sys_user_admin(request):
    ...
    for user in users:
        if user.props.id == request.user.id:
            user.is_self = True
        try:
            user.self_usage = wingufile_api.get_user_self_usage(user.email)
            user.share_usage = wingufile_api.get_user_share_usage(user.email)
            user.quota = wingufile_api.get_user_quota(user.email)
        except:
            user.self_usage = -1
            user.share_usage = -1
            user.quota = -1
...
def user_info(request, email):
    if request.method == 'POST':
        ...
        if f.is_valid():
            ...
            try:
                wingufile_api.set_user_quota(email, quota)
            except:
    ...
    owned_repos = wingufile_api.get_owned_repo_list(email)

    quota = wingufile_api.get_user_quota(email)
    quota_usage = 0
    share_usage = 0
    my_usage = 0
    my_usage = wingufile_api.get_user_self_usage(email)
    if CALC_SHARE_USAGE:
        try:
            share_usage = wingufile_api.get_user_share_usage(email)
    ...
    in_repos = wingufile_api.get_share_in_repo_list(email, -1, -1)
