...
from winguhub.forms import RepoNewDirentForm, RepoRenameDirentForm
from winguhub.views import get_repo_dirents
from winguhub.views.repo import get_nav_path, get_fileshare, get_dir_share_link
import winguhub.settings as settings
from winguhub.signals import repo_created
from winguhub.utils import check_filename_with_rename, star_file, unstar_file
...
def get_dirents(request, repo_id):
    ...
    try:
        dirents = wingufile_api.list_dir_by_path(repo_id, path.encode('utf-8'))
    ...
    for dirent in dirents:
        if stat.S_ISDIR(dirent.mode):
            ...
            if dir_only:
                ...
                try:
                    dirent_dirents = wingufile_api.list_dir_by_path(repo_id, dirent_path.encode('utf-8'))
                except SearpcError, e:
                    dirent_dirents = []
...
def get_group_repos(request, group_id):
    ...
    repos = wingufile_api.get_group_repo_list(group_id_int)    
    repo_list = []
    for repo in repos:
...
def get_my_unenc_repos(request):
    ...
    repos = wingufile_api.get_owned_repo_list(request.user.username)
    repo_list = []
    for repo in repos:
        if repo.encrypted:
            continue
...
def list_dir(request, repo_id):
    ...
    if repo.encrypted and not wingufile_api.is_password_set(repo.id, username):
...
def new_dir(repo_id, parent_dir, dirent_name, username):
    ...
    # create new dirent
    try:
        wingufile_api.post_dir(repo_id, parent_dir, dirent_name, username)
    except SearpcError, e:
        result['error'] = str(e)
...
def new_file(repo_id, parent_dir, dirent_name, username):
    ...
    # create new dirent
    try:
        wingufile_api.post_empty_file(repo_id, parent_dir, dirent_name, username)
    except SearpcError, e:
        result['error'] = str(e)
        return HttpResponse(json.dumps(result), status=500,
                            content_type=content_type)
...
def delete_dirent(request, repo_id):
    ...
    # delete file/dir
    try:
        wingufile_api.del_file(repo_id, parent_dir, dirent_name, username)
        return HttpResponse(json.dumps({'success': True}),
                            content_type=content_type)
    except SearpcError, e:
        logger.error(e)
...
def mv_file(src_repo_id, src_path, dst_repo_id, dst_path, obj_name, username):
    ...
    try:
        ...
        wingufile_api.move_file(src_repo_id, src_path, obj_name,
...
def get_contacts(request):
    ...
    contact_list = [] 
    from winguhub.avatar.templatetags.avatar_tags import avatar
    for c in contacts:
...
def check_sub_repo(request, repo_id): 
    ...
    try:
        sub_repo = wingufile_api.get_virtual_repo(repo_id, path, request.user.username)
    except SearpcError, e:
        result['error'] = e.msg
        return HttpResponse(json.dumps(result), status=400, content_type=content_type)
...
def create_sub_repo(request, repo_id):
    ...
    try:
        sub_repo_id = wingufile_api.create_virtual_repo(orig_repo_id, orig_path,
