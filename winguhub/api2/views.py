...
from winguhub.base.models import FileDiscuss
from winguhub.share.models import FileShare
from winguhub.views import access_to_repo, validate_owner, is_registered_user, events, group_events_data, get_diff
from winguhub.utils import gen_file_get_url, gen_token, gen_file_upload_url, \
...
from winguhub.group.forms import MessageForm
from winguhub.notifications.models import UserNotification
from winguhub.utils.paginator import Paginator
from winguhub.group.models import GroupMessage, MessageReply, MessageAttachment
from winguhub.group.settings import GROUP_MEMBERS_DEFAULT_DISPLAY
from winguhub.group.signals import grpmsg_added, grpmsg_reply_added
from winguhub.group.views import group_check
from django.template import Context, loader, RequestContext
from django.template.loader import render_to_string
from django.shortcuts import render_to_response
from django.http import HttpResponseRedirect
from winguhub.utils import EVENTS_ENABLED, TRAFFIC_STATS_ENABLED, api_convert_desc_link, api_tsstr_sec, get_file_type_and_ext
...
from winguhub.shortcuts import get_first_object_or_none
...
class Repo(APIView):
    ...
    def delete(self, request, repo_id, format=None):
        ...
        if not wingufile_api.is_repo_owner(username, repo_id):
            return api_error(status.HTTP_403_FORBIDDEN, \
                    'Only library owner can perform this operation.')

        wingufile_api.remove_repo(repo_id)
...
class BeShared(APIView):
    ...
    def get(self, request, format=None):
        username = request.user.username
        shared_repos = []
        shared_repos += wingufile_api.get_share_in_repo_list(username, -1, -1)
        ...
        # Get group repos, and for each group repos...
            for r_id in get_group_repoids(grp.id):
                ...
                if wingufile_api.is_repo_owner(username, r_id):
                    continue
                ...
                r.share_type = 'group'
                r.user = wingufile_api.get_repo_owner(r_id)
        ...
        if share_type == 'personal':
            if not is_registered_user(user) :
                return api_error(status.HTTP_400_BAD_REQUEST,
                                 'User does not exist')
            try :
                from_email = wingufile_api.get_repo_owner(repo_id)
            ...
            try:
                wingufile_api.group_share_repo(repo_id, int(group_id),
